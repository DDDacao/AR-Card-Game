...
> **更新日期**：2026-07-12（新版战斗 HUD + 意图/回合文案 + 奖励用真实 Card 预制体已验收；弱点仍有 bug 暂挂）  
...
玩法闭环可演示。不要破坏 BattleFlowManager / 符匣顺序 / Card 预制体分层。
当前可优先：弱点精修，或真机全流程回归。
```
...
| 三色弱点 + QTE | 🔧 | 逻辑可用；挂怪物 Prefab 头骨、仅小光球；**位置/可见度仍有 bug** |
...
| 三关战役 + 换模 | ✅ | `ActiveMonster`；编辑器 Ellen 占位正常 |
| 灼烧构筑 | ✅ | 烈火 1 / 炼火 2；敌方行动后结算；引火引爆 |
| 开始界面 | ✅ | 唯一 `PF_StartIntro` |
| 手机 AR | ✅ | 用户已验证扫图；Editor `skipARForEditor` |

**相对策划案仍缺**：弱点精修、精准圆环 QTE、Boss 多阶段、更丰富封印演出、受击音效/震动、真机全流程回归、「未识别不开战」门闩（可选）。
...
### 7.1 弱点（仍有 bug，未阻塞 UI）

- 逻辑可用；Collider 在怪物 Prefab 头骨下；无粒子（避免红方块）。  
- 头骨：`Vespomorph_Head` / `CAVECRAWLER_HEAD` / `Drackmahre_ Head`（有空格）。  
- 用户反馈：位置/嵌模/明显度仍可能有问题。  
- 调参：Prefab 上 `Weakness_*` 或 `WeaknessAnchorSetup.PlaceOutsideHead`。
...
- [ ] **弱点精修**（位置、可见度、三关手感、真机可点）  
- [ ] **真机全流程回归**（扫图→三关→两次奖励→通关/重试）  
- [ ] UI 安全区 / 结束回合在真机上的位置  
...
**首选：** 修弱点（Prefab 头骨偏移 / `WeaknessAnchorSetup`，Play 与真机都可点）。  
...
|----|------|
| 新版战斗 HUD | ✅ 用户验收整体 OK |
| 意图文案 | ✅ 仅一行意图名；中文字体/缩放修复 |
| 意图底框 | ✅ 随文字收紧 |
| 回合状态 | ✅ 回合数+阶段+牌堆/手牌 |
| BossHealth 非均匀 scale | ✅ 名/意图脱离该父节点 |
| 奖励 UI | ✅ **实例化 Card.prefab** + Init 数据 |
| 奖励被遮罩挡住 | ✅ 临时 Screen Space Camera + 透明 Panel + CardCamera 置顶 |
| 弱点 | 🔧 仍挂起，待下轮 |
